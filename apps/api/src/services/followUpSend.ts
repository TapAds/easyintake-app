...
export async function deliverFollowUpSms(params: {
  ...
  const body = getFollowUpSmsBody(params.templateId, params.firstName);
  ...
  if (wantGhl && !forceTwilio && params.ghlContactId) {
    if (sticky === "email" && params.applicantEmail?.trim()) {
      const { messageId } = await sendGhlConversationEmail(params.ghlLocationId, {
        ...
        html: getFollowUpEmailHtml(params.templateId, params.firstName),
  ...
  const twilioResult = await sendFollowUpSms(params.phone, params.templateId, params.firstName);
  return {
    provider: "twilio",
